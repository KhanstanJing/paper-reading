...
二、字典中的密钥应该用相同或者相似的编码器表示，这样它们和序列的对比才能是一致的（？）

（20240204update：现在有了更好的理解。

机器学习的是特征，特征是编码器通过喂进来的数据提取出来的

那么对于每一个新喂进来的数据，机器要：首先提取特征；其次学习并更新参数。

而我们的机器学完之后，要达到给它一份数据，它提取特征后，和学到而后放进字典中的特征，即密钥进行比对，从而给反馈

而在学的时候，字典中的密钥需要用编码器提取特征，）
