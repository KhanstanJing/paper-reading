...
这里有些地方看不懂的原因应该主要是对于自回归模型不太熟悉（autoregressive model），不过目前只要知道它是一种时间序列模型，用于预测时间序列数据中的未来值。在自回归模型中，当前时间点的观测值被认为是过去时间点的值的线性组合，加上一个随机误差项。换句话说，自回归模型假设当前观测值与之前的观测值之间存在某种自相关性就可以了

这里就进一步对于其有了更加深刻的理解，就是每一个$c_t$都是由从当前观测$x_t$生成潜在表示$z_t$和之前的预测$c_{t-1}$生成的预测，**每一个这样的预测都是由当前的特征和之前的预测所组成的**

目前我的看法是，这样的预测的生成方式则是通过自回归模型，生成后，以往的方式可能是根据所生成的预测**完全对于原始数据进行建模，而这种生成会有大量的计算**

但是这里作者采用了一个密度比例，这个比例的计算方式就是把当前的**预测**和后面的已经抽取出来的**潜在表示**进行某种计算（当然这里进行了一个线性变换后计算），**因此这里不需要生成方面的计算，而只是把预测和已经提取出来的信息进行某种计算**，这个计算的结果就代表了两个信息之间的关系

总之，之前是在比对模型的预测结果和正确结果的差距，类似于查字典，但是作者是通过首先让模型生成的预测包含以往的预测，并且衡量该生成的预测和未来的信息的某种类似于相关性的东西

#### 五、作者总结

这种看法应该是正确的，因为作者在下方总结了一下，使用密度比例$f_k(x_{t+k},c_t)$和用编码器推断$z_{t+k}$，他们将模型从对$x_{t_k}$的高维分布建模中释放出来了（“relieve the model from modeling the high dimensional distribution $x_{t_k}$”）

尽管作者不能直接对于$p(x)$和$p(x|c)$估值，但是可以从这些分布中采样，这样就可以用基于对比目标值和随机采取负样本的NCE（Noise-Contrastive Estimation）和Importance Sampling（！等会儿看一下）这样的技巧（注意NCE和IS是在哪里用到的）

##### 潜在表示和预测的用处

作者还在这里讲解了一下潜在表示和预测的用处，他说这两个都可以作为下游任务的（读到这要不是看到下游任务我差点都忘了这是无监督学习）的表示，并且两个东西各有不同的使用场景

自回归模型的输出$c_t$，最好用在一些来自过去的额外上下文很有用的情况，例如语音识别中，这个时候$z_t$的感受野可能不能包含足够的信息去捕捉语音内容（？不是很清楚为什么，这一块没有了解过）

编码器的输出$z_t$，最好用在不需要额外的上下文的情况中（很好理解，因为编码器只是对于每一段语音进行输出，并没有像自回归模型一样把当前信息和之前的信息联系起来）

如果说下游任务需要为整个序列提供一整个表示，那么我们可以从所有地方任意选取$z_t$或者$c_t$的表示进行池化，比如图像分类任务（？没读懂这里的“提供一整个表示”“进行池化”是什么意思，原文是这样“If the downstream task requires one representation for the whole sequence, as in e.g. image classification, one can pool the representations from either $z_t$ or $c_t$ over all locations”）

##### 注记

作者说在他们提出的框架中，任何编码器和自回归模型都可以使用（在这里可以看出作者之前说的“通用”“普遍”算法是什么意思了）

基于简单性的考虑，作者比较倾向于一些标准架构，类似于“步幅卷积层”（原文内容为“strided convolutional layer”，意思就是按照一步一步移动的卷积神经网络，可能是之前都这么叫），编码器用ResNet块；自回归模型用的是GRUs（！看一下）

作者也提到了一些自回归建模中的成绩，例如掩码卷积架构（masked convolutional architecture）或者自注意力网络（self-attention network）也都能取得更多的成果
